// The typed shape the whole app consumes. The adapter is the ONLY module that
// knows the raw data.gov.il API shape; everything downstream uses these types.
// A returned VehicleReport always represents a FOUND vehicle — "not found" is
// modeled as the adapter returning null.
...
export interface VehicleReport {
  ...
  ownerCount: Enrichment<{ count: number; type?: string }>; // separate dataset → "לא זמין"

  // Safety
  recalls: Enrichment<{ open: number; items: string[] }>; // separate dataset → "לא זמין"

  sources: SourceRef[];
  fetchedAt: string; // ISO timestamp — also serves as source_updated_at on cache hits
}
